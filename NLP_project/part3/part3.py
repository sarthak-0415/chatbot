####.....ASSUMPTIONS.........
#the match number is given as digits e.g. 'match 3'.....
# if no country given assume country = india.....
# if match no not given and player name not given too....
...
def match_words(words, keywords):
	...
	keywords = keywords[0]
	temp = wn.synsets(keywords)
	if len(temp) > 0:
		keyword_temp = str(temp[0])
		keyword_temp1 = keyword_temp.split("'")
		keyword = keyword_temp1[1]
	else:
		#print keywords_word
		#print temp
		score = 0
	...
	word_pos = word.split('.')	
	keyword_pos = keyword.split('.')
	
	if word_pos[1] == keyword_pos[1] :
		word_compare = wn.synset(word)
		keyword_compare = wn.synset(keyword)
		...
		score = word_compare.wup_similarity(keyword_compare)
		if len(str(score)) != 4:
			##print 'score::'
			#print score
			return score
